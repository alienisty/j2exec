...
public final class CommandBuilder {
	...
	public static ParsedCommand parse(@NonNull String str) {
		...
		parsing: for (int cIdx = 0, strLength = str.length(); cIdx < strLength;) {
			...
			case ' ':
				if (!quoting) {
					if (prefix != null) {
						// argument found
						arguments.add(new Argument(prefix, str.substring(
								tokenStart, cIdx - 1), index));
		...
		if (prefix != null) {
			arguments
					.add(new Argument(prefix, str.substring(tokenStart), index));
		} else if (str.length() - tokenStart > 1) {
	...
	public static void main(String[] args) {
		ParsedCommand command = parse("cmd -s {?} -d \"a \\\"quoted\\\" srting\"   -g={?} -f");
	}
